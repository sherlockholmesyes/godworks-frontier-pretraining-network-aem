...
EVIDENCE_VERIFY_REPORT_ARTIFACT = "evidence_verify_report.json"
...
def _load_manifest(manifest_path: str | Path) -> dict[str, Any]:
    ...
    validate_or_raise(data, "evidence_bundle_manifest.schema.json")
    return data
...
def verify_evidence_bundle(manifest_path: str | Path) -> dict[str, Any]:
    ...
    count_ok = manifest.get("artifact_count") == len(manifest.get("artifacts", []))
    ...
    validate_or_raise(report, "evidence_verify_report.schema.json")
    return report


def write_evidence_verify_report(
    manifest_path: str | Path,
    output_path: str | Path | None = None,
) -> dict[str, Any]:
    report = verify_evidence_bundle(manifest_path)
    manifest_file = Path(manifest_path)
    output = Path(output_path) if output_path is not None else manifest_file.parent / EVIDENCE_VERIFY_REPORT_ARTIFACT
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Verify AEM evidence bundle manifest hashes")
    parser.add_argument("manifest", help="evidence_bundle_manifest.json path")
    parser.add_argument("output", nargs="?", help="optional evidence_verify_report.json output path")
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.output:
        report = write_evidence_verify_report(args.manifest, args.output)
    else:
        report = verify_evidence_bundle(args.manifest)
